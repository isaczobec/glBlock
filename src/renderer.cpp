...
void renderer::Camera::CalculateViewMatrix() {
	...
	m_viewMatrix = projectionMatrix * viewMatrix; // set view matrix
}

void renderer::Camera::UpdateTransform(glm::vec3 deltaPosition, glm::vec3 deltaRotation) {
	m_position += deltaPosition;
	m_rotation += deltaRotation;
	m_rotation.x = glm::clamp(m_rotation.x, -glm::pi<float>() / 2, glm::pi<float>() / 2); // clamp x rotation
...
glm::mat4x4* renderer::Camera::GetViewMatrix() {
	return &m_viewMatrix;
}
